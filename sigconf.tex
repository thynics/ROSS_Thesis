...
We evaluate the method by separating observations with an actionable HEFT--MSI conflict from weak- or no-opportunity controls, and by using exact gate removal to expose over-optimization. A qualified 75-run matrix spans five workload observations on one 8-GPU NVIDIA L4 node. Prefix-safe reduces modeled copy by 85.8\% and 38.8\% and improves over HEFT by 1.405x and 1.173x on the two effective problem cases, Cholesky and LU. CG and MiniWeather expose no modeled-copy opportunity and remain neutral/non-degrading at 1.005x and 0.982x; FDTD improves by 1.064x without a copy signal and is therefore reported but not counted as mechanism evidence. No workload-level geomean regresses. Exact gate removal is 1.087x faster than prefix-safe on Cholesky, exposing conservative gate cost, whereas prefix-safe is 1.208x faster on LU despite no-prefix removing more copy, exposing harmful over-optimization. In a controlled CG case, one fixed-successor SALC intervention conditionally eliminates seven attributed peer-copy requests and is followed by 27 downstream placement changes. Eleven of fifteen workload--scheduler group medians miss our predeclared 5\% scheduler-share target; this is a prototype limitation rather than a universal incremental prefix-gate cost.
...
\item
  We define a problem-oriented evaluation that separates artifact validity, modeled-copy opportunity, latency response, and gate-removal risk. A qualified 75-run, five-workload matrix identifies two effective problem cases, two safe no-opportunity controls, one latency gain without mechanism signal, no workload-level regression, and opposite gate-ablation outcomes.
\end{enumerate}
...
Workload names do not define the method's scope. Table~\ref{tab:workloads} samples distinct problem conditions in the primary repeated matrix: Cholesky and LU expose substantial modeled-copy opportunity, CG and MiniWeather test behavior when that opportunity is absent, and FDTD adds a regular stencil graph whose latency can change without a modeled-copy response. Standard CG also supplies prior identity-control and certificate evidence. The Cholesky and LU configurations were frozen after one-run exploration and are therefore mechanism-bearing cases, not an unbiased prevalence survey. We use them to test the response to the HEFT--MSI conflict, not to claim that dense factorization is the method's target domain.
...
\input{figures/allbench_problem_boundary_table.tex}

Table~\ref{tab:allbench-problem-boundary} reports the predeclared classification for all five observations. ``P material'' counts paired H--P ratios of at least 1.03x; the P/H and \(T_N/T_P\) columns are paired geomeans. Cholesky and LU are the effective problem cases: prefix-safe reduces scheduler-modeled copy by 85.8\% and 38.8\%, improves over HEFT by 1.405x and 1.173x, and is materially faster in all five pairs for each observation. This is positive evidence for the general HEFT--MSI remedy under an observed copy opportunity, not evidence that the method targets two benchmark names.

CG and MiniWeather have identical modeled-copy totals under H, P, and N. Their P/H geomeans, 1.005x and 0.982x, lie in the predeclared neutral/non-degrading band. They therefore act as safe no-opportunity controls. FDTD has a 1.064x P/H geomean, but P changes no modeled copy and only three of five pairs reach the material threshold while two are slower. We retain the latency observation but do not count it as evidence for the copy-aware mechanism. Across the five workload-level geomeans there is no regression below 0.97x.
...
\subsection{Historical strong-opportunity gate isolation}\label{clean-fresh-calibration-repeated-gate-isolation}

The earlier 30-run block supplies source-distinct supporting evidence for the two effective problem cases; it is not pooled with the primary matrix. All executions pass the provenance, completeness, correctness, calibration, and deterministic-mechanism checks. HEFT (H), prefix-safe (P), and no-prefix (N) share one fresh calibration per benchmark; P and N differ only in prefix admission. Table~\ref{tab:gate-triplet-repeat} reports direction-explicit ratios to avoid reciprocal ambiguity.
...
\textbf{Problem coverage, selection, and arrival model.} The admission rule is defined independently of workload identity. The five-workload matrix now distinguishes two effective modeled-copy-opportunity cases, two safe no-opportunity controls, and one latency response without mechanism signal, but it still does not estimate prevalence across applications. The two strongest configurations were selected after one-run exploration; their repeated results establish behavior when the conflict is present rather than an unbiased frequency. Null and future negative observations remain part of the method boundary rather than failed attempts to win every benchmark. The repeated DAGs are fully materialized before deferred submission. Decisions are still made one task at a time, but the evaluation does not validate dynamic task arrival, and the positive SALC case relies on a graph-visible fixed successor. Results come from one 8-L4 node, one size per workload, and five repeats.
...
A current-task EFT optimum can be a poor residency decision when a read--write placement moves the only valid version away from a future consumer. Prefix-safe copy-aware HEFT is a general, benchmark-independent admission rule for this HEFT--MSI conflict: it accepts lower-copy placements only within numerical tolerance of the current modeled-prefix finish bound and does not predict the future graph. In the qualified 75-run primary matrix, it reduces modeled copy and improves latency in all ten pairs across the two effective problem cases, remains neutral/non-degrading in two no-opportunity controls, and produces no workload-level geomean regression. FDTD's gain without a copy response is reported but excluded from mechanism evidence.
...
Eleven of fifteen primary group medians miss our predeclared 5\% scheduler-share target, although all CG executions meet it. The present contribution is therefore a general enforceable invariant with condition-specific positive evidence, explicit null evidence, and a prototype-level deployment limitation. Broader systems, scales, and independently selected problem conditions are required to estimate how often the conflict occurs and to search for regressions; dynamic-arrival evaluation, stronger repeated locality baselines, and lower scheduler cost remain necessary before making a universal performance claim.
