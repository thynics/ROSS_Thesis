...
\item
  We define a problem-oriented evaluation that separates artifact validity, modeled-copy opportunity, latency response, and gate-removal risk. A clean 30-run gate-only block supplies two strong-opportunity observations with opposite ablation outcomes, together with an explicit negative scheduler-share result and coverage boundary.
\end{enumerate}
...
Standard CG & \texttt{\detokenize{16777216 40 10}} & FP64 & 3,040 & 40 static contexts with bounded iterations; excluded from HEFT-relative performance after identity failure \\
\bottomrule
\end{tabularx}
\end{table*}

Workload names do not define the method's scope. Table~\ref{tab:workloads} samples distinct problem conditions: Cholesky and LU provide the currently qualified repeated observations with substantial modeled-copy opportunity, MiniWeather provides a secondary weak/no-opportunity observation, standard CG supplies identity-control and certificate evidence, and FDTD adds a regular stencil graph to the expanded matrix. The Cholesky and LU configurations were frozen after one-run exploration and are therefore mechanism-bearing cases, not an unbiased prevalence survey. We use them to test the response to the HEFT--MSI conflict, not to claim that dense factorization is the method's target domain.
