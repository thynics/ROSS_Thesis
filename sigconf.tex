...
A qualified 180-run matrix covers five workloads on one 8-GPU NVIDIA L4 node. Within each workload, a Williams design balances HEFT (H), a same-path identity control (I), two transparent locality baselines (W and O), prefix-safe placement (P), and exact gate removal (N) for block, position, and first-order carryover. On the two workloads where P changes placement, Cholesky and LU, it reduces modeled copy by 81.0\% and 38.8\% and improves latency over H by 1.313x and 1.177x. P materially outperforms both locality baselines on Cholesky and the owner baseline on LU; it is within the predeclared neutral range of the weighted baseline on LU. Gate removal exposes both sides of the safety--opportunity tradeoff: it helps Cholesky but harms LU. The other three workloads retain identical H/P/N placements and serve as timing controls. Nineteen of thirty workload--scheduler group medians miss our predeclared 5\% scheduler-share target, a prototype limitation. A separate controlled fixed-successor trace confirms the motivating migrate--return mechanism but is not used as performance evidence.
...
Online heterogeneous schedulers must decide where to execute a task before the rest of a dynamic graph has necessarily become runnable. CUDASTF implements the Sequential Task Flow model over CUDA and automates dependencies and data movement for annotated logical data~\citep{augonnet2024cudastf,nvidia2026cudastf}. The fork studied here uses a \emph{HEFT-style current-task device selector}: for the runtime-supplied next task, it estimates each device's earliest finish time (EFT), including current-input availability, and chooses the minimum. Unlike canonical offline HEFT, which orders tasks by upward rank before EFT-based processor selection~\citep{topcuoglu2002heft}, the fork does not recompute task order. This distinction matters: our contribution refines device placement, not DAG ordering.

The local objective is nevertheless incomplete when placement changes data state. CUDASTF's logical-data layer uses an explicit \emph{software} MSI state: a write-like access leaves the new version on the selected GPU and invalidates stale instances elsewhere~\citep{nvidia2026cudastf}. This is not a claim about a hardware cache-coherence protocol. A placement that wins a fraction of a millisecond for the current task can consequently move a large mutable version away from the GPU where a later task consumes it. The later task either copies the version back or follows it and moves other inputs. In both cases the locally best EFT decision can initiate a residency cascade whose traffic is larger than the immediate gain. We use ``HEFT--MSI conflict'' as shorthand for this interaction between the runtime's HEFT-style selector and its software residency model.
...
This study revisits the placement layer of our prior thesis~\citep{chengli2026multiobjective}. That system used a conventional HEFT-relative residual gate and a contextual bandit; here we isolate a translation-invariant current-prefix contract, add exact gate removal and identity controls, and replace the earlier fixed-order evidence with a counterbalanced design.
...
For current task \(v\), let \(A_k(v,d)\) be the latest availability time among its read and read--write inputs on \(d\). A local Modified or Shared instance incurs no transfer; otherwise availability includes a copy from a valid instance. The fork's current-task estimate follows HEFT's EFT structure~\citep{topcuoglu2002heft}:
...
Among prefix-safe alternatives, we retain the prior placement layer's copy-aware selectivity~\citep{chengli2026multiobjective}. Define
...
The frozen configuration inherits \(K_{\min}=Q_{\min}=0.001\) ms, \(\tau=0.25\) when \(c(v)>0.60\) and \(0.15\) otherwise, and \(\lambda_Q=0.35\) from that prior layer~\citep{chengli2026multiobjective}. These values are shared by every benchmark, were frozen before the repeated confirmation, and are not claimed optimal. Eligible alternatives are ranked by
...
Figure~\ref{fig:prefix-safe-selector} summarizes the concrete state consumed by this rule: the existing evaluator supplies HEFT's baseline row, residency state supplies modeled copy, prefix admission selects at most one alternative, and committing that row updates the ready-time and MSI state used by the next decision.

For controlled mechanism isolation, the source-matched \texttt{no-prefix} ablation retains the same HEFT-style baseline, candidate set, copy and queue gains, thresholds, score, and tie breaks, but disables only the prefix admission rule. It consequently has no prefix contract. This exact removal distinguishes the gate's effect from the prior conventional relative-EFT comparator~\citep{chengli2026multiobjective}, which changes the timing rule itself.
...
\begin{table}[t]
\centering
\caption{Controlled fixed-successor mechanism trace. Traffic values use binary units.}\label{tab:controlled-cg-certificate}
\footnotesize
...
\Description{Controlled CG migrate--return trace. HEFT moves a 128 MiB mutable version from its owner on GPU 0 to task 46 on GPU 1, then six fixed-device reads return it. The fixed-successor SALC certificate keeps task 46 owner-local. Runtime attribution is conditional and trace-on latency is excluded.}
\caption{Controlled CG migrate--return certificate. HEFT moves the 128 MiB version away from its owner; six fixed-device reads return it. SALC's one-hop certificate keeps task 46 owner-local. Runtime attribution is conditional and trace-on latency is excluded.}
\end{figure*}
...
\Description{A ready CUDASTF task enters the existing candidate evaluator. A table records each GPU's predicted finish, modeled input-copy cost, and secondary score. The HEFT baseline row and a lower-copy candidate row feed prefix-safe admission; the committed task enters a GPU queue and updates ready-time and MSI residency state for the next decision.}
\caption{CUDASTF selector state. HEFT supplies baseline \(b\); prefix-safe may admit lower-copy \(d^\star\). Committing the selected row updates ready-time and MSI state for the next task.}\label{fig:prefix-safe-selector}
\end{figure*}
...
The primary matrix ran on one dual-NUMA Europa node with eight NVIDIA L4 GPUs (23,034 MiB each), driver 595.71.05, CUDA 12.4.131, GCC 13.3.0, CMake 4.3.4, CUDASTF/CCCL 3.2.0.0~\citep{nvidia2026cccl320}, and a C++17 Release build. The supporting replication used an earlier, source-distinct snapshot on the same platform; the artifact manifest binds every result to its exact source revision. GPUs 0--3 and 4--7 form separate NUMA-local PCIe groups; cross-group traffic traverses the system interconnect. A nearby metadata snapshot identifies a dual-socket Intel Xeon Gold 6548N host, but its clock and persistence readings are not treated as steady-state attestations.
...
\begin{table*}[t]
\centering
\caption{Workload configurations and correctness contracts. All workloads use FP64.}\label{tab:workloads}
\footnotesize
...
\toprule
Workload & Configuration & Tasks & Correctness contract \\
\midrule
...
FDTD, CG, and Cholesky are task-graph adaptations of the CUDASTF sources distributed with CCCL v3.2.0~\citep{nvidia2026cccl320}; LU is our project-specific no-pivot tiled companion. Cholesky and LU invoke FP64 cuBLAS and cuSOLVER kernels~\citep{nvidia2024cublas,nvidia2024cusolver}, while MiniWeather is a six-region task-graph adaptation of the public miniWeather mini-app~\citep{norman2018miniweather}. For Cholesky and LU, the correctness check applies three fixed \(\{-1,+1\}\) probes to the reconstructed factorization, normalizes the worst projected residual by \(\lVert A\rVert_\infty\), and also requires the full output checksum; it is not an exhaustive matrix-norm residual.
...
I exactly reproduces H task and dependency-check counts, placement digest, modeled-copy accounting, scheduler-context count, and correctness checksum for all 30 workload--block pairs. Its I/H geomeans are 1.024x (FDTD), 1.001x (CG), 1.035x (Cholesky), 1.018x (LU), and 1.000x (MiniWeather). These nonzero same-decision differences quantify scheduler-path and residual system effects; in particular, Cholesky crosses the 1.03x threshold without changing placement. Independently, H/P/N are placement- and copy-identical on FDTD, CG, and MiniWeather. We consequently apply algorithmic labels only when the compared mechanism outputs differ.
...
\caption{Primary five-workload P/H evidence. (a) Six within-block ratios and geometric means; filled/open points denote different-/identical-placement cases. (b) Median modeled copy; values are not physical traffic.}\label{fig:primary-counterbalanced-results}
\end{figure*}

Table~\ref{tab:counterbalanced-baselines}(a) reports all five workloads and all four P-relative comparisons, rather than showing only the effective cases; Figure~\ref{fig:primary-counterbalanced-results} exposes the six individual P/H pairs and the associated copy totals. P changes H's placement only on Cholesky and LU. It reduces scheduler-modeled copy from 128.55 to 24.41 GB on Cholesky (81.0\%) and from 148.77 to 90.98 GB on LU (38.8\%). The corresponding P/H speedups are 1.313x (range 1.296--1.324x) and 1.177x (1.171--1.186x); all six pairs agree in each case. This is positive evidence for the remedy when a prefix-admissible HEFT--MSI opportunity is observed, not a prevalence claim about the two benchmark names.

The simpler locality baselines do not explain away the result. P materially outperforms W and O on Cholesky and O on LU. Its 1.020x LU advantage over W is below the frozen 1.03x threshold and is therefore competitive rather than material. P also models less copy than either baseline in both cases. Conversely, W changes CG placement without reducing its 42.95 GB modeled copy and is 1.841x slower than P, showing why indiscriminate copy weighting is not a sufficient substitute for bounded admission.

\paragraph{Unchanged-placement controls.}
FDTD, CG, and MiniWeather have identical H/P/N placement digests and modeled-copy totals; their parenthesized ratios in Table~\ref{tab:counterbalanced-baselines}(a) are therefore timing controls, not locality effects. FDTD and MiniWeather are identical across all six treatments. On CG, W alone changes placement, yet leaves modeled copy at 42.95 GB and is 1.841x slower than P. FDTD's six P/H ratios span 0.801--1.157x despite identical decisions, exposing the instability of this short interval. We therefore make neither locality-speedup nor universal non-regression claims from the parenthesized observations.
...
Table~\ref{tab:counterbalanced-baselines}(b) places the source-distinct 30-run block beside the primary results; the two blocks are not pooled. The replicated P/H geomeans differ from the primary estimates by only 0.002x on Cholesky and 0.006x on LU, while copy reductions differ by 0.1 and 0.6 percentage points. The opposite P/N outcomes also repeat: exact gate removal helps Cholesky, whereas P protects LU. All five replication pairs agree in each displayed direction. The block passes provenance, completeness, correctness, calibration, and deterministic-mechanism checks. Its agreement is corroborative; the counterbalanced matrix closes its fixed-order threat and adds repeated locality baselines.
...
\textbf{Traffic attribution.} Runtime-issued transfer accounting validates CUDASTF copy requests, not bus transactions. The controlled CG trace has no independent runtime version epoch, so its seven attributed requests are conditional on the task/data/fixed-successor join. Device-level copy activity and timing would require CUPTI activity tracing or an equivalent profiler~\citep{nvidia2026cupti}; such records still do not by themselves establish physical PCIe/NVLink link traffic.
...
\begin{acks}
OpenAI Codex~\citep{openai2026codex} assisted with adversarial manuscript review, experiment-protocol drafting, repository analysis, and language editing throughout Sections~1--9. The authors retain responsibility for the research design, source code, experimental execution, validation, and all claims.
\end{acks}

\section*{Artifact Availability}\label{artifact-pointers}

The implementation, harnesses, 180-run counterbalanced primary matrix, historical 30-run supporting block, raw qualified results, generated interpretation tables, and fail-closed verification scripts are preserved together. A machine-readable manifest records exact source and result revisions while keeping primary timings separate from instrumented forensic and provenance-incomplete diagnostics. The working repository is presently access-controlled, so a reviewer-accessible archival URL must replace this sentence before submission.
