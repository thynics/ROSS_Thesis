...
We compare HEFT, prefix-safe, and exact gate removal on two selected dense DAG configurations on one 8-GPU NVIDIA L4 node. Across 30 provenance- and correctness-checked executions, prefix-safe outperforms HEFT in all five Cholesky pairs (1.315x geomean, 81.1\% modeled-copy reduction) and all five LU pairs (1.171x, 39.4\%). Gate removal helps Cholesky by 1.162x but loses to prefix-safe on LU by 1.205x despite removing 95.7\% more modeled copy, supporting a workload-dependent risk control rather than a universal speedup source. In a controlled CG case, one fixed-successor SALC intervention conditionally eliminates seven attributed peer-copy requests and is followed by 27 downstream placement changes. Every group, including HEFT, misses our predeclared 5\% scheduler-share target; this is a prototype limitation, not incremental prefix-gate overhead.
...
This paper asks a narrow practical question: can an online scheduler suppress harmful locality deviations without attempting to solve the remaining DAG? Our main answer is a prefix-bounded refinement. We first retain HEFT's device as a baseline. We then consider devices that reduce modeled current-input copy and admit one only if its finish fits the baseline's current-prefix finish bound up to numerical tolerance. This method is broad: it applies to ordinary tasks and does not require a special mutable-chain label. It also remains intentionally local. The safety condition says nothing about the makespan of the unscheduled suffix.
...
\item
  We report a clean 30-run gate-only evaluation that exposes opposite Cholesky and LU ablation outcomes, together with an explicit negative scheduler-share result and coverage boundary.
\end{enumerate}

The resulting claim is deliberately smaller than ``automatic chain discovery.'' Prefix safety mitigates traffic without recognizing a chain; fixed-successor SALC certifies one graph-visible edge pattern. Neither method proves a suffix or global makespan improvement.
...
The Cholesky and LU configurations in Table~\ref{tab:workloads} are the primary repeated workloads. They were selected after one-run exploration because they expose locality-sensitive placement choices, then frozen before confirmation; they are not an unbiased benchmark survey or evidence that the conflict is prevalent. MiniWeather is retained only as a secondary neutral case, and standard CG only as a control/certificate source.
...
Prefix-safe beats HEFT in all ten pairs, with a 1.241x geomean over ten paired observations from the two selected configurations. Exact gate removal has opposite effects: it improves Cholesky in every pair but loses to prefix-safe on LU in every pair even while removing substantially more modeled copy. This is consistent with the gate rejecting a harmful aggressive policy on LU while conservatively rejecting profitable aggression on Cholesky. It supports an enforceable modeled-prefix invariant and workload-dependent risk control, not a chain detector or universal speedup mechanism.
...
Finally, a one-read--write/unique-owner restriction covers every task in the selected dense configurations and produces the same placements as the broad method. It therefore does not identify chains; prefix-safe remains a broad locality refinement.
...
A current-task EFT optimum can be a poor residency decision when a read--write placement moves the only valid version away from a future consumer. Prefix-safe copy-aware HEFT addresses this symptom without predicting the future graph: it accepts lower-copy placements only within numerical tolerance of the current modeled-prefix finish bound. On two selected dense configurations, prefix-safe beats the runtime HEFT baseline in all ten pairs, with a 1.241x geomean over the paired observations and zero modeled prefix damage.

Exact gate removal helps Cholesky in every pair, while prefix-safe beats no-prefix on LU in every pair despite no-prefix removing substantially more modeled copy. The result supports a workload-dependent risk control, not a universal speedup mechanism. A fixed-successor SALC case goes one step closer to the motivating chain: one intervention conditionally eliminates seven attributed peer-copy requests and is followed by 27 downstream placement changes. It certifies the constructed one-hop inequality, not complete chain discovery or end-to-end causality.

Every clean group also misses our predeclared 5\% scheduler-share target, including HEFT, while prefix-safe's absolute scheduler cost is lower than HEFT's. The present contribution is therefore an enforceable modeled invariant with positive selected-workload evidence and an explicit prototype-level deployment limitation. Dynamic-arrival evaluation, stronger repeated locality baselines, broader workloads and systems, and lower scheduler cost are required before making a general performance claim.
