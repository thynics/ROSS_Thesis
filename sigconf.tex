...
FDTD, CG, and MiniWeather have identical H/P/N placement digests and modeled-copy totals. FDTD and MiniWeather also have identical outputs for all six treatments. Their P/H ratios (1.022x, 0.997x, and 1.000x) and P/N ratios (1.002x, 0.998x, and 1.004x) are timing controls, not locality effects. FDTD's six P/H ratios span 0.801--1.157x despite identical decisions, exposing the instability of this short interval. We therefore make neither locality-speedup nor universal non-regression claims from these rows.
...
The implementation, harnesses, 180-run counterbalanced primary matrix, historical 30-run supporting block, raw qualified results, generated interpretation tables, and fail-closed verification scripts are preserved at result-bearing commit \texttt{58cb4c57a}. Its manifest records the full source-under-test SHA beginning \texttt{e3cfeaf3}; primary and historical timings remain separate from instrumented forensic and provenance-incomplete diagnostics. The working repository is presently access-controlled, so a reviewer-accessible archival URL must replace this sentence before submission.
